-- =============================================
-- FRESH INSTALL - Game Rotation System
-- =============================================
-- This will clean up any previous attempts and install fresh
...
DROP FUNCTION IF EXISTS public.cleanup_old_game_rotations() CASCADE;

-- Create game_rotations table
CREATE TABLE public.game_rotations (
...
-- Create indexes
CREATE INDEX idx_game_rotations_user_id ON public.game_rotations(user_id);
CREATE INDEX idx_game_rotations_refresh_at ON public.game_rotations(refresh_at);
...
CREATE FUNCTION public.get_user_game_rotation(
  p_user_id UUID,
...
BEGIN
  ...
  SELECT 
    ...
    v_current_rotation.rotation_number AS rotation_number,
    v_current_rotation.refresh_at AS refresh_at
...
-- Cleanup function
CREATE FUNCTION public.cleanup_old_game_rotations()
RETURNS INTEGER AS $$
DECLARE
...
-- Success!
SELECT '✅ Game rotation system installed successfully!' as status;
SELECT '🎮 Total enabled games: ' || COUNT(*)::TEXT as games_count FROM public.mini_games WHERE enabled = TRUE;

-- Fix claim_pending_rewards to not fail if email template doesn't exist
...
BEGIN
  ...
  LOOP
    ...
    CASE v_pending.reward_type
      ...
      WHEN 'stamps' THEN
        -- Add stamps (without notes column)
  ...
  -- Try to send notification (but don't fail if template doesn't exist)
  IF v_claimed_count > 0 THEN
    ...
    EXCEPTION WHEN OTHERS THEN
      -- Email failed, but that's okay - don't rollback the claim
      RAISE NOTICE 'Email notification failed: %', SQLERRM;
    END;
  END IF;
